...
def predictemotion():
    ...
    nparr_image = np.frombuffer(string_image,np.unit8)
    image = cv2.imdecode(nparr_image,cv2,IMREAD_GRAYSCALE)
    ...
    return jsonify({"emotion": "face not detect","x1":str(0),"y1":str(0),"x2":str(0),"y2":str(0)}), 200
